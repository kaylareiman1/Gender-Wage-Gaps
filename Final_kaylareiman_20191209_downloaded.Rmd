...
Acknowledgement: Thank you to classmates Erika Montana and Betty Liu for informing my conceptual work on this project.
...
#### **Part 1: Import the data and clean it**
...
3) This allows for cleaner graphics, showing the relationship between variables that applies to the majority of the sample.
...
```{r}
childpoverty.palette <- scale_fill_manual(values = c("mediumpurple1", "mediumpurple1", "#c1cdcd"))
...
race.bi <- ggplot(data = race.bi.conf, aes(x = race, y = AvgIncome, fill = I("mediumpurple1"))) +
  geom_bar(stat = "identity") +
  xlab("Race in 2000") + 
  ylab("Average Income ($)") +
  ggtitle("Bivariate Overview \nIncome by Race") + 
...
# Plot, with error bars
racegapplot <- ggplot(data = racegap, aes(x = race, y = income.gap, fill = I("mediumpurple1"))) +
  geom_bar(stat = "identity") +
  xlab("race") + 
  ylab("Income gap($)") +
  ggtitle("Gender Analysis\nIncome Gap by Race") + 
...
```{r}
#Create marital palette w/ shades of green
marital.palette <- scale_fill_manual(values = c("mediumpurple1", "mediumpurple1", "mediumpurple1", "#c1cdcd"))
...
```{r}
highestgradeuni <- ggplot(survey.notopcode, aes(x=highestgrade, fill = I("mediumpurple1")))
...
# Plot, with error bars
ggplot(data = gap.data.conf, aes(x = `as.factor(highestgrade)`, y = AvgIncome, fill = I("mediumpurple1"))) +
  geom_bar(stat = "identity") +
...
# Plot, with error bars
ggplot(data = educationgap_notopcode, aes(x = `as.factor(highestgrade)`, y = income.gap,fill = I("mediumpurple1"))) +
  geom_bar(stat = "identity") +
  xlab("education") + 
  ylab("Income gap($)") +
...
ggplot(data = educationgap_all, aes(x = `as.factor(highestgrade)`, y = income.gap,fill = I("mediumpurple1"))) +
  geom_bar(stat = "identity") +
  xlab("education") + 
  ylab("Income gap($)") +
...
```{r}
edu.binuni <- ggplot(survey.notopcode, aes(x=edu.bin, fill = I("mediumpurple1")))
...
edu.bingapplot <- ggplot(data = edu.bingap, aes(x = edu.bin, y = income.gap, fill = I("mediumpurple"))) +
  ...
  xlab("edu.bin") + 
  ylab("Income gap($)") +
  ggtitle("Gender Analysis\nIncome Gap by Binned Education") + 
  guides(fill = FALSE) +
  geom_errorbar(aes(ymax = upper, ymin = lower), width = 0.1, size = 1) +
  xlab("Binned Education ") + 
  ylab("Average Income ($)") +
...
grid.arrange(edu.bin.bi+  ylim(-10000, 50000), edu.bingapplot, ncol = 2)
```
...
industryuni <- ggplot(industry.bi.conf, aes(x=industry, y = count, fill = I("mediumpurple1")))
...
```{r}
industry.bi1 <- ggplot(data = industry.bi.conf, aes(x = industry, y = AvgIncome, fill = I("mediumpurple1"))) +
  geom_bar(stat = "identity") +
  xlab("Industry") + 
  ylab("Average Income ($)") +
